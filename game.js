...
/* ======================================================
   ORIENTATION BUTTONS + AUTO-DETECT ROTATION
   ====================================================== */

/** Apply portrait (1-column) layout */
...
/** Apply landscape (2-column) layout */
...
/** Detect phone/tablet rotation automatically */
...
/** Manually click Portrait button */
document.getElementById("portraitBtn").addEventListener("click", () => {
  applyPortraitLayout();
});

/** Manually click Landscape button */
document.getElementById("landscapeBtn").addEventListener("click", () => {
  applyLandscapeLayout();
});

/** Listen for screen rotation (mobile / tablet) */
window.addEventListener("resize", detectOrientation);

/** Initial layout check on load */
window.addEventListener("load", detectOrientation);


/* ======================================================
   RESPONSIVE RACETRACK — ALWAYS FITS SCREEN
   ====================================================== */
function resizeRacetrack() {
  const track = document.getElementById("racetrack");

  // Portrait: racetrack full width, limited height
  if (document.body.classList.contains("portrait-mode")) {
    track.style.width = "95%";
    track.style.height = (window.innerHeight * 0.25) + "px";
  }
  // Landscape: racetrack tall + rightside
  else {
    track.style.width = "100%";
    track.style.height = (window.innerHeight * 0.75) + "px";
  }
}

// Run once on load
window.addEventListener("load", resizeRacetrack);
// Run after every resize
window.addEventListener("resize", resizeRacetrack);
